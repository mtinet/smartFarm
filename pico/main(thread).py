...
def fanAndPWM_control():
    while True:
        # Handling the fan based on temperature
        temperatureValue = round(sensor.temperature)
        db_fan_value = urequests.get(url + ".json").json()['smartFarm']['fan']
        time.sleep(0.5)

        if temperatureValue > temperatureStandardValue or db_fan_value == 1:
            fan.value(1)
        else:
            fan.value(0)

        # Handling the water pump based on moisture
        moistureValue = round(moisture.read_u16() / 65535 * 100)

        if moistureValue < moistureStandardValue:
            pwm.duty_u16(65025)  # 10% duty cycle
        else:
            pwm.duty_u16(0)
...
# 스레드를 시작합니다.
_thread.start_new_thread(fanAndPWM_control, ())
